...
@section('title','| Manage Create Folder')
...
@section('content')
	<div class="row">
		<div class="col-md-8 myWrap">
			<h1><a class="pointer" id="menu-toggle2"><span class="fas fa-folder-open mr-4"></span>Create A New Folder</a></h1>
			<hr>

			{!! Form::model($folder,['route'=>['folders.store', $folder->id], 'method'=>'PUT', 'data-parsley-validate'=>'', 'files'=>true]) !!}
{{--
			{!! Form::open(['route'=>'folders.store', 'data-parsley-validate'=>'', 'files'=>true]) !!}
--}}
			...
			{{ Form::select('user_id', $users, auth()->user()->id, ['class'=>'form-control custom-select', 'placeholder'=>'Select a User...', 'data-parsley-required'=>'']) }}			
		...
		<div class="col-md-4">
			...
			<div class="mt-3">
				<div id="myImgOld-1" style="display:none">
					{{-- Used in Edit only 												--}}
					<img src="{{ asset('images/'.$folder->image) }}" width="100%" />
				</div>
